...
Note there is **no TypeScript installed** in this project, so `tsconfig.json`'s strict settings are honoured only by the editor — no `tsc`/`astro check` runs in CI.
